...
\item Instantaneous: not accounting for long-term performance \begin{tiny}(more on that later)\end{tiny} 
\end{itemize}
\end{block}
}
\only<3->{
...
\item Violation $\rightarrow$ Utility = $-\infty$ 
\item Instantaneous: not accounting for long-term violations \begin{tiny}(more on that later)\end{tiny} 
\end{itemize}
\end{block}
}
\only<5->{
...
\textbf{Remarks}:
\begin{itemize}
\item Optimization does not yield an action but a ``map" info$\rightarrow$action
...
\end{block}
\flushright ...when the future is uncertain
}
...
\item Imperfect model: complex relationship optimal plan / optimal policy (but recently understood)
\end{itemize}
...
\begin{center}
\visible<3->{More formally: $\mathrm{P}\left[\vect s_{k+i}\,|\, \vect\pi,\vect s_k\right] = \mathrm{P}\left[\vect s_{k+i}\,|\, \vect\pi,\vect s_k, \vect s_{k-1},\ldots, \vect s_0\right]$ for any $i>0$}\\
\vspace{.25cm}
\visible<4->{
...
\end{itemize}
\item \visible<2->{\textbf{Build policy} from $\vect\pi_{\vect \theta}^\star=\argmax_{\vect \pi}\,\hat J_{\vect\theta}\left(\vect \pi\right)$, i.e optimal for the model}
...
\end{align*}
\vspace{-.75cm}
 \visible<1-4>{\flushright fitting real-world data}
\end{minipage}
 };
...
\textbf{Build Policy}
 \vspace{-0.2cm}
\begin{align*}
\vect\pi_{\vect \theta}^\star%(\vect{s})
\end{align*}
 \vspace{-0.5cm}\\
...
\only<5->{\begin{center} In other words, the best model tuning for fitting the data is not (necessarily$^1$) the best model tuning for taking decisions.\end{center}
