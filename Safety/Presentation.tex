...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{itemize}
    ...
    \uncover<5-> {
    \item Instead of a single density, use a distribution of densities.  }
  \end{itemize}
...
\scriptsize\textbf{Remarks:}
\begin{itemize}
  \item Accounts for \textbf{epistemic uncertainty} from limited data.  
  \item Distributional robust MDP: less pessimistic than robust MDP. 
  \item Probabilistic Safety: confidence level based on model undertainty??
  \item Distributional view is applied to risk-sensitive MDP via a distribution of returns (e.g distributional RL).
  \item \textcolor{red}{connection to bayesian vs freequentist view??}
\end{itemize}
}
