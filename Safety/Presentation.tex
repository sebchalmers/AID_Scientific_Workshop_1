...
\author[Akhil S Anand]{\large Dr. Akhil Anand}
  ...
  \begin{itemize}
    \visible<1->{
      \item Re-plan at each stage, hedging against worst case uncertainties in the future.
        ...
        \visible<2->{
    \item \textbf{Robust MPC:} rolling horizon, worst-case tubes, online re-plan.}
        \visible<4->{
    \item \textbf{Scenario trees \/ Stochastic programming:} hedge decisions across branched futures.}
        \visible<10->{ 
    \item \textbf{Sampling:} Monte Carlo and \emph{MPPI} for large-scale problems.}
  \end{itemize}
  \visible<11->{
    \vspace{0.3cm}
  \textbf{Remarks:}
...
\begin{columns}[T,onlytextwidth]
  ...
  \textbf{In words}
  \begin{itemize}
 \item  Instead of constraints, the utility function itself is altered to contain a risk measure of the return. 
...
\begin{itemize}
 \item Views risk from through the lens of utility (not undesirable events).
...
\vspace{-0.25cm}
\begin{center}
\textbf{Key idea}: substract a strong penalty to the utility when a violation occurs
\end{center}
\vspace{-0.1cm}
\begin{itemize}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{itemize}
    ...
    \item At each step, if the proposed action would leave the safe set, a small QP minimally adjusts it to be safe.
  \end{itemize}
  \vspace{-0.35em}
  \end{alertblock}
  ...
  \begin{overlayarea}{\linewidth}{0.42\textheight}
    ...
    \only<4>{
      ...
      \end{figure}
        ...
        \caption*{\scriptsize Vector filed without CBF filter.}
      \end{figure}
}
  \end{overlayarea}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{itemize}
    ...
    \visible<4-> {
    \item Instead of a single density, distribution of densities}
