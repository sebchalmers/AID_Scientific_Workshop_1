...
\tikzstyle{SA_block} = [rectangle, draw, fill=none,  text width=4cm,  text height=3cm,  text centered]
...
\node [SA_block] (SA0) {\textbf{State action space}

};

%\draw[blue,thick] ($(SA0.north east)+(+0,1)$)  rectangle ($(SA0.south west)+(-0,-1)$);

% \node [anchor=$SA0.north east)+(+0,1)$] (SA1) {
%$\vect s,\vect a$
% };

% \node at (1,1) [above=2pt+3pt,draw] {above};

 % \node [below left =10mm of SA0.north east] (SA1) {$\vect s,\vect a$};
  
 \node[  below right = 15mm and 10mm of SA0.north west] (SA1) {$1:\vect s,\vect a$};
 \node[  below right =  5mm and 10mm of SA0.north west] (SA2) {$2:\vect s,\vect a$};
  \node[  below right =  20mm and 25mm of SA0.north west] (SA3) {$3:\vect s,\vect a$};

  
   \node [Dist_block, right of=SA0, node distance = 5cm] (Dist0) {
 \includegraphics[width=1\textwidth,clip]{Codes/Epistemic/Dist_s_a_1.eps}
 };
 
 
   \node [Dist_block, above of=Dist0, node distance = 2.7cm] (Dist1) {
 \includegraphics[width=1\textwidth,clip]{Codes/Epistemic/Dist_s_a_2.eps}
 };
 

  \node [Dist_block, below of=Dist0, node distance = 2.7cm] (Dist2) {
 \includegraphics[width=1\textwidth,clip]{Codes/Epistemic/Dist_s_a_0.eps}
 };
...
\]
%\vspace{-0.35em}
 $\rho$ is a risk measure (e.g., VaR, Entropic, CVaR). 
\end{block}}
\end{columns}
...
\begin{itemize}
  \item Often possile to solve using classical methods such as DP.
...
\begin{frame}{\normalsize Out-Of-Distribution ``Guard" Methods}
\footnotesize
\textcolor{red}{SG can do}
