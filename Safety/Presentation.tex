...
\begin{columns}[T,onlytextwidth]
  ...
  \vspace{-0.25em}
  \begin{itemize}
    \item Define a safe set $\mathcal{C} = \{x : h(x)\ge 0\}$ using a barrier function $h$.
    \item Keep the state inside $\mathcal{C}$ by enforcing a simple inequality at each step.
    \item If a suggested action is unsafe, a small QP adjusts it to the nearest safe action.
  \end{itemize}
  \vspace{-0.35em}
  \end{alertblock}
