...
splus = np.array([[],[]])
for n in range(0,NMC):
    dist = np.random.choice([0,1],p=[.8,.2])
    draw = True
    while draw:
        Noise = dist*np.matmul(B,np.random.normal([0.05,-0.05],std,2)) + (1-dist)*np.random.normal(0,2*std,2)
...
#ax1.plot(splus[0,:],splus[1,:],color='k',linestyle='none',marker='.',markersize=2)
...
figID.savefig('PessimisticOneStep0.eps', bbox_inches='tight')#,dpi='figure', format=None,metadata=None, pad_inches=0.1,facecolor='auto', edgecolor='auto', backend=None) #,
...
figID.savefig('PessimisticOneStep1.eps', bbox_inches='tight')#,dpi='figure', format=None,metadata=None, pad_inches=0.1,facecolor='auto', edgecolor='auto', backend=None, bbox_inches='tight') #,
