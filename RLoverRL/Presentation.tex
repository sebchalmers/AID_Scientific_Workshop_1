...
\begin{itemize}
  ...
  \item Acuuracy and robustness does not guarantee optimal real-world performance.
...
{\scriptsize \textit{Graphic:} $\delta\theta$ changes edge weights (transition probs), so the Bellman backup shifts via both reward and where we go next.}
\end{columns}
\begin{itemize}
