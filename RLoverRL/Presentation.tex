...
\title[Introduction to Safe Decision Making]{Introduction to Theory of Safe Decision Making}
...
\date[18 September 2025]{$1^\mathrm{st}$ AID Scientific Workshop, Trondheim}
...
\vspace{0.5cm}
\center
\includegraphics[width=0.6\textwidth,clip]{Figures/image.png}
...
% \newcommand{\Sys}{\vspace{-0.25cm}\includegraphics[width=2\textwidth,clip]{Figures/image.png}}
...
\tikzstyle{Sys_block} = [
  ...
  minimum size=0pt % allow exact fit
]
...
\node [Sys_block] (sys0) at (8,0) {
  \includegraphics[width=0.35\textwidth]{Figures/image.png}
};
...
%\center
\only<2->{
\textbf{Safety in the math world}
}  %Stay within certain ``sets"
\vspace{.1cm}
...
\begin{frame}{\normalsize MDPs with Hard Constraints}
\footnotesize
\vspace{-0.5cm}
\begin{columns}
\column{0.5\textwidth}
...
%\begin{columns}
%\column{0.6\textwidth}
\visible<3->{\textbf{Build policy} using either:
\begin{itemize}
...
\only<13->{
\textbf{Epistemic problem}: build container... 
\begin{itemize}
\item from data, but it may expand with more data and safety is ``asymptotic", as data $\rightarrow\infty$
\item \visible<14->{``theoretically" (from knowledge), but can be impractically pessimistic}
\end{itemize}
}
\end{overlayarea}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{itemize}
    \visible<1->{
      \item Re-plan at each stage, hedging against worst case uncertainties in the future
    }
        \visible<2->{
    \item \textbf{Robust MPC:} constraint tightening}
        \visible<4->{
    \item \textbf{Scenario trees \/ Stochastic programming:} hedge decisions across all branched futures.}
        \visible<10->{ 
    \item \textbf{Sampling:} Monte Carlo for large-scale problems}
  \end{itemize}
  \visible<11->{
    \vspace{0.3cm}
  \textbf{Remarks:}
  \begin{itemize}
    \item In practice: robust optimization = \textbf{min--max}: maximize utility over scenarios that avoid worst-case violation.
    \item Hard to establish guarantees for nonlinear systems
    \item Scenario explosion.
    \item Persistent safety (\emph{recursive feasibility}) under re-planning.
  \end{itemize}}

  % Right column: overlay figures
  \column{0.40\textwidth}
\begin{overlayarea}{0.4\textwidth}{0.7\textheight}
  \only<1>{
      \includegraphics[width=2\textwidth,clip]{Figures/Chess.pdf}
  }
  \only<2>{
        \begin{figure}
        \centering
        \includegraphics[width=3\textwidth]{Codes/RobustMPC/IllustrationRobustMPCPreTerminal.eps}
      \end{figure}
}
\only<3>{
        \begin{figure}
        \centering
        \includegraphics[width=3\textwidth]{Codes/RobustMPC/IllustrationRobustMPCTerminal.eps}
      \end{figure}
}
\only<4>{
  \input{Codes/Scenariotree/scenario_1.tex}
}
\only<5>{
  \input{Codes/Scenariotree/scenario_2.tex}
}
\only<6>{
  \input{Codes/Scenariotree/scenario_3.tex}
}
\only<7>{
  \input{Codes/Scenariotree/scenario_4.tex}
}
\only<8>{
  \input{Codes/Scenariotree/scenario_5.tex}
}
\only<9->{
  \input{Codes/Scenariotree/scenario_6.tex}
}
...
\begin{columns}[T,onlytextwidth]
  ...
  \textbf{In words}
  \begin{itemize}   
 \item No notion of Constraints
 \item Maximize the utility of bad outcomes
  \item Modify the utility to account for risk
  \end{itemize}
  \end{alertblock}
  }
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{Codes/Risk/RS_CVaR_Saddle.pdf}
  \end{figure}
  }
  ...
  \textbf{Formally}
  \[
    \pi^* = \arg\max_{\pi}\; \rho\!\Big(\sum_{t=0}^{\infty} \gamma^t r_t\Big)
  \]
  \scriptsize $\rho$: VaR, CVaR, Entropic Risk.
  \end{block}
  }
  ...
  \begin{figure}
    \centering
    \includegraphics[width=1.05\linewidth]{Codes/Risk/RiskFunctional_Rho.pdf}
  \end{figure}
  }

\end{columns}

% -------- Bottom full-width remarks
\uncover<5->{
\vspace{-0.03cm}
\scriptsize\textbf{Remarks:}
\begin{itemize}
 \item Views risk through the lens of utility (not undesirable events).
  \item Risk-sensitive criteria do \emph{not} guarantee a bound on violation.
  \item  Cares only about the average severity of bad outcomes in the tail and not about the rest.
\end{itemize}
}
...
% % ===============================
% \begin{frame}{\normalsize Methods for Risk-Sensitive RL}
% \footnotesize
% \begin{itemize}
%   \uncover<1->{
%   \item \textbf{Utility-based (Exponential / Entropic risk)}  
%         Reformulate Bellman operator with exponential utility \(U(x) = -\exp(-\beta x)\).  
%         \textit{Solved with DP.}}
% \uncover<2->{
%   \item \textbf{Coherent risk measures (CVaR, VaR, spectral)}  
%         Express objective a risk measure.  
%        \textit{Solved with covexification, policy gradient methods.}
  
%        {\centering
% $ \mathrm{CVaR}_{\alpha}(Z)=\min_{\eta\in\mathbb R}\Big[\eta+\tfrac{1}{1-\alpha}\,\mathbb{E}\big[(Z-\eta)_+\big]\Big]$}
%        }
% \uncover<3->{
%   \item \textbf{Distributional approaches}  
%         Learn the full distribution of returns instead of just the mean.  \textit{Solved with distributional RL.}}
% \uncover<4->{
%   \item \textbf{Constraint-based formulations}  
%         Risk as explicit constraints, e.g.\ $\mathrm{CVaR}(\text{cost}) \le d$.  
%         \hfill \textit{Solved with Lagrangian relaxation, primal–dual policy gradient.}}
% \end{itemize}

% \vspace{0.3cm}

% % --- Simple VaR / CVaR equations linked to left image ---
% \normalsize
...
% \uncover<2->{
% \centering
% %\includegraphics[width=0.85\linewidth]{Codes/Risk/RS_CVaR_Saddle.pdf}\\
% \scriptsize CVaR tail beyond $\eta$ shapes the policy.}

% \column{0.5\textwidth}
% \uncover<4->{
% \centering
% %\includegraphics[width=0.85\linewidth]{Codes/Risk/DistributionalRL.pdf}\\
% \scriptsize Distributional RL: decisions from full return distribution.}
% \end{columns}
...
\vspace{-.2cm}
\begin{align*}
\vect\pi^\star = \argmax_{\vect\pi}&\quad\mathrm E\left[ \sum_{k=0}^\infty \gamma^k r\left(\vect s_k,\vect a_k\right)\right]\\
...
\vspace{-0.25cm}
\begin{center}
\textbf{Key idea}: Add a strong penalty to the utility when a violation occurs
\end{center}
\vspace{-0.1cm}
\begin{itemize}
\item \visible<3->{May start with a ``fully unsafe" policy}
\item \visible<4->{Following gradient $\nabla_{\vect\theta}J\left(\vect\pi_{\vect\theta}\right)$ reduces violations  }
\item \visible<10->{But safety improvement ``stalls" as violations become more rare}
\end{itemize}
\visible<21->{\textbf{Difficulties:}}
\begin{itemize}
\item \visible<21->{Noisy gradient when violations become rare}
\item \visible<22->{``Hard"safety is never reached (asymptotic)}
\item \visible<23->{The higher (probabilistic) safety we want the more data we need}
\end{itemize}
...
\column{0.4\textwidth}
  \begin{overlayarea}{\textwidth}{.55\textheight}
	...
	\includegraphics[width=1\textwidth,clip]{Codes/RL/RLSafety17.pdf}
        ...
        {
        \center
	\includegraphics[width=1\textwidth,clip]{Codes/RL/RLSafety18.pdf}
	}
\end{figure}
\end{overlayarea}
\end{columns}
...
\visible<2->{
\begin{block}{}
\textbf{Train policy }$\vect\pi_{\vect\theta}$ via evaluations of $\nabla_{\vect\theta}J\left(\vect\pi_{\vect\theta}\right) $ and
\vspace{-.1cm}
\begin{align*}
J\left(\vect\pi_{\vect\theta}\right) = \mathrm E\left[  \sum_{\text{time}} \left(\text{Utility} - \cmatr{ccc}{0&\text{if}&\text{Constraint ok}\\p &\text{if}&\text{Constraint not ok}}\right)\right]
\end{align*}
\flushright  for $p$ large
\end{block}}
...
\begin{alertblock}{}
\center
Unrealistic in the real world, it's a simulation-based process
\end{alertblock}
...
\textbf{Remarks}:
\begin{itemize}
\item Often close to practice, e.g., flight envelope protection in modern airplane, semi-autonomous driving in cars, etc
...
% =======================
  ...
  \vspace{-1cm}
    ...
    \only<2->{
    \item Use a barrier function $h(x)$ so that staying safe means staying in set $\mathcal{C}=\{x: h(x)\ge 0$\} }
    \only<4->{ 
    \item CBF modifies the nomainal decision by solving a QP \[
    u^\star(x) = \arg\min_{u}\tfrac12\|u-u_{\mathrm{des}}(x)\|_2^2
    \quad\text{s.t. derivative of the h(x) along the vector field} \geq 0
  \]}
  \end{itemize}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{overlayarea}{\linewidth}{0.42\textheight}
    ...
    \only<7>{
      ...
      \end{figure}
        ...
        \begin{figure}
        \centering
        \includegraphics[width=0.55\linewidth]{Codes/CBF/unsafe_vectorfield.eps}
        \caption*{\scriptsize Vector field without CBF filter.}
      \end{figure}
}
  \end{overlayarea}
  ...
  \begin{overlayarea}{\linewidth}{0.42\textheight}
    ...
    \only<4>{
        \begin{figure}
        \centering
        \includegraphics[width=0.55\linewidth]{Codes/CBF/safe_vectorfield.eps}
        \caption*{\scriptsize vector field with CBF filter.}
      \end{figure}
}
  \end{overlayarea}
\end{columns}
...
\scriptsize
\begin{itemize}
  \item The barrier function and safe set $\mathcal{C}$ are typically {constructed from domain knowledge} (physics, rules, safety envelopes).
  \item Requires a (possibly simplified, conservative) {system model}: {robust} approach.
\end{itemize}
}
\endgroup
\end{frame}
...
% Low density
 ...
 \includegraphics[width=1\textwidth,clip]{Codes/Epistemic/Dist_s_a_0.pdf}
  ...
  \visible<6->{
    \node [Text_block,  below left =  -7mm and 0mm of SA0.south east] (Text0) {
    \textbf{Remarks}
    \begin{itemize}
    \scriptsize
    \item \textcolor{red}{$\vect s,\vect a$} not much visited $\rightarrow$ Low \textcolor{blue}{data} about $\vect s_+$
    \item \visible<7->{Low data about $\vect s_+$ $\rightarrow$ uncertainty about the distribution \textit{itself}: epistemic uncertainty (unknown unknown)}
    \item  \visible<8->{Continuous state-action problems $\rightarrow$ single data point per $\vect s,\vect a$}
    \item  \visible<9->{Rely on regularity across $\vect s,\vect a$!!}
    \end{itemize}
 };}
...
\begin{frame}{\normalsize Distributional View on MDPs}
\footnotesize
\setlength{\columnsep}{1.1cm}
...
\begin{columns}[T,onlytextwidth]
  ...
  \visible<1->{
  \begin{itemize}
    \item True dynamics are unknown; only samples or estimates available}
    \visible<4-> {
    \item Instead of a single distribution, a family of distributions
    }
    \visible<10->{\item {Knowledge embedding:} encode physics/rules as priors or constraints on models}
    \visible<11->{\item {Bayesian view:} Data updates the prior belief about the model providing a posterior }
    \visible<12->{\item {Building $\mathcal P$:}  ensembles, parametric, non-parametric}
  \end{itemize}
  ...
  % Figure left (progressive few-data ambiguity visuals)
  \begin{overlayarea}{0.9\linewidth}{0.45\textheight}
    \only<2>{\centering\includegraphics[width=0.9\linewidth]{Codes/DR/DistViewFew_0.pdf}}
    \only<3>{\centering\includegraphics[width=0.9\linewidth]{Codes/DR/DistViewFew_1.pdf}}
    \only<4>{\centering\includegraphics[width=0.9\linewidth]{Codes/DR/DistViewFew_2.pdf}}
    \only<5>{\centering\includegraphics[width=0.9\linewidth]{Codes/DR/DistViewFew_3.pdf}}
    \only<6>{\centering\includegraphics[width=0.9\linewidth]{Codes/DR/DistViewFew_4.pdf}}
    \only<7>{\centering\includegraphics[width=0.9\linewidth]{Codes/DR/DistViewFew_5.pdf}}
    \only<8>{\centering\includegraphics[width=0.9\linewidth]{Codes/DR/DistViewFew_6.pdf}}
    \only<9->{\centering\includegraphics[width=0.9\linewidth]{Codes/DR/DistViewFew_7.pdf}}
  \end{overlayarea}
...
\scriptsize\textbf{Remarks:}
\begin{itemize}
  \item Accounts for \textbf{epistemic uncertainty} from limited data; connects to probabilistic safety via confidence levels.
  \item Distributionally Robust Optimization for worst-case robustness against model misspecification (more conservative).
  \item Expectation over distribution of distributions: some form of discretization. Bayesian marginalization or Scenario discretization
\end{itemize}
}
...
\footnotesize
\textbf{Utility + model may ``push" a problem to get ``Out-Of-Distribution"}
\begin{itemize}
\scriptsize
\item Unwarranted extrapolation / generalization
\item Optimization often exploits the model ``weaknesses" 
\end{itemize}
\begin{columns}[t]
\column{0.6\textwidth}
\visible<2->{
...
\vspace{-.25cm}
\begin{align*}
\textcolor{myGreen3}{\vect\pi^\star} = \argmax_{\vect\pi}\,\mathrm E\left[ \sum_{k=0}^\infty \gamma^k\left(\vect s_k^2+\vect a_k^2\right)\right]
\end{align*}
\end{block}}
\visible<3->{
\textbf{Key idea} 
\begin{itemize}
\item Max utility while penalizing OOD. E.g.
\begin{alertblock}{}
\vspace{-.25cm}
\begin{align*}
\textcolor{myMagenta}{\vect\pi^\text{OOD}} = \argmax_{\vect\pi}\,\mathrm E\left[ \sum_{k=0}^\infty \gamma^k\left(\vect s_k^2+\vect a_k^2-\omega\cdot\varphi\left(\vect s_k,\vect a_k\right)\right)\right]
\end{align*}
\end{alertblock}
\begin{flushright} where $\varphi$ is the data density (e.g. kernel)\end{flushright}}
\item \visible<4->{As we get more data, $\vect\pi^\text{OOD}\rightarrow\vect\pi^\star$ (where it is useful)}
\end{itemize}
...
\item Knowledge-driven model can still be data-driven, but with simpler structure than AI model (e.g. linear)
...
\footnotesize
\begin{center}
\textit{We have discussed on the key concepts of safe decision making}
\end{center}

\begin{overlayarea}{\textwidth}{0.75\textheight}
\only<2-3>{
\begin{block}{}
\textbf{RA6 (Extrapolation \& Generalization)} 
\begin{itemize}
\item Data-based models have epistemic uncertainty
\item Decision making will often push the models outside their validity (unwarranted extrapolation)
\item How to manage this problem better is a key RA in AID
\end{itemize}
\end{block}
}
\only<3>{
\begin{block}{}
\textbf{RA4 (Knowledge embedding)}
\begin{itemize}
\item Knowledge is often key to manage epistemic uncertainty 
\item How to embed knowledge in AI for Decision to manage extrapolation better is a key RQ in AID
\end{itemize}
\end{block}
}
\only<4-5>{
\begin{block}{}
\textbf{RA5 (Training models for decision)} 
\begin{itemize}
\item Best model fit $\neq$ best model for decision
\item The problem of generating relevant scenarios (or probability mass) for safe decision making is strongly connected (same theory)
\item How to train models to give us predictions conducive to safety is a key RQ in AID
\item Especially open for probabilistic safety guarantees
\end{itemize}
\end{block}
}
\only<5>{
\begin{block}{}
\textbf{RA7 (AI and formal decision tools)} 
\begin{itemize}
\item Formal decision tools offer a pathway to ensure safe decisions
\item E.g. robust repeated planning
\item How to combine these tools in a good way with AI is an active research topic. Especially how to make it work in practice. 
\end{itemize}
\end{block}
}
\end{overlayarea}
